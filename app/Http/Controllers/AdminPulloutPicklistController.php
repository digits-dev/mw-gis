<?php namespace App\Http\Controllers;
	...
	class AdminPulloutPicklistController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        */
			...
			if(CRUDBooster::isSuperadmin() || in_array(CRUDBooster::myPrivilegeName(), ["Online WSDM"])){
				$this->addaction[] = ['title'=>'Print Pick List','url'=>CRUDBooster::mainpath('print-picklist').'/[st_document_number]','icon'=>'fa fa-print','color'=>'info','showIf'=>"[status]=='FOR PICKLIST'"];
			}
	    ...
	    public function hook_query_index(&$query) {
	        ...
	        if(!CRUDBooster::isSuperadmin()){
				...
				elseif(in_array(CRUDBooster::myPrivilegeName(), ["Warehouse","Warehouse Online","RMA"])){
					...
					$query->select('pullout.st_document_number','pullout.wh_from','pullout.wh_to','pullout.status','pullout.created_date')
					->whereIn('pullout.stores_id',CRUDBooster::myStore())->where('pullout.status','FOR PICKLIST')->distinct();
				}
			}
			else{
			    $query->select('pullout.st_document_number','pullout.wh_from','pullout.wh_to','pullout.status','pullout.created_date')
				->where('pullout.status','FOR PICKLIST')->distinct();
			}
	    }
	    ...
	    */
		...
		public function getPrint($st_number)
		{
			...
			DB::table('pullout')->where('st_document_number', $st_number)->update([
			    'picklist_date'=>date('Y-m-d'),
				'status'=>'FOR PICK CONFIRM'
			]);
