<?php namespace App\Http\Controllers;
	...
	class AdminHqDeliveryController extends \crocodicstudio\crudbooster\controllers\CBController {
...
// 			$this->addaction[] = ['title'=>'Details','url'=>CRUDBooster::mainpath().'/details/[dr_number]','icon'=>'fa fa-eye','color'=>'primary'];
	    ...
	    */
	    public function hook_query_index(&$query) {
	        ...
	        if(!CRUDBooster::isSuperadmin()){
				...
				$query->select('dr_number','customer_name','status','data_pull_date')
					// ->whereRaw('SUBSTRING(customer_name, -3,  3) != RTL')
					// ->whereRaw('SUBSTRING(customer_name, -3,  3) != FRA')
					->where('customer_name',$store->bea_mo_store_name)
					->orWhere('customer_name',$store->bea_so_store_name)
					->distinct();   
			}
			else{
				$query->select('dr_number','customer_name','status','data_pull_date')->distinct();
			}   
	    }
