...
class AdminStoreTransferGisController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function saveCreateGisST(Request $request)
	{
		...
		//pos pull header and reference number counter
		$ref_code_counter = CodeCounter::where('id', 3)->value('pullout_refcode');
		$mwts_ref_no = 'MWSTS-' . str_pad($ref_code_counter, 7, 0, STR_PAD_LEFT);
		...
		if (empty($mwts_ref_no)) {
			//back to old form
			CRUDBooster::redirect(CRUDBooster::adminpath('store_pullout'), 'Failed! No MWSTS has been created.', 'danger')->send();
		}
		...
		//pos_pull_headers records
		$header = [
			'st_document_number' => $st_ref_no,
			'reference_number' => $mwts_ref_no,
			'status' => self::Pending,
			'memo' => $request->memo,
		...
		$headerRecordId = DB::table('pos_pull_headers')->insertGetId($header);
		...
		foreach ($request->digits_code as $key_item => $value_item) {
			...
			//lines
			$stDetails = [
				'pos_pull_header_id' => $headerRecordId,
			...
			$posPullRecord = DB::table('pos_pull')->insert($stDetails);
		}
