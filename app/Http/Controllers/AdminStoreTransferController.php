	...
	class AdminStoreTransferController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			if(!in_array(CRUDBooster::myPrivilegeName(),["LOG TM","LOG TL","Warehouse","RMA", "Operations Manager", "Area Manager"])){
				$this->addaction[] = [
					...
					'icon'=>'fa fa-times',
					'color'=>'danger',
					'showIf'=>"[status]==".TransactionStatus::PENDING."",
					'confirmation'=>'yes',
	    ...
	    public function hook_query_index(&$query) {
			...
			if(!CRUDBooster::isSuperadmin()){
				...
				else if (in_array(CRUDBooster::myPrivilegeId(), [28])){
	    ...
	    public function hook_row_index($column_index,&$column_value) {	        
	    	//Your code here
			if($column_index == 5){
				if($column_value == TransactionStatus::PENDING){
					$column_value = '<span class="label label-warning">'.TransactionStatus::PENDING.'</span>';
				}
				else if($column_value == TransactionStatus::CONFIRMED){
					$column_value = '<span class="label label-warning">'.TransactionStatus::CONFIRMED.'</span>';
				}
		...
		public function autoRejectHandCarry(){
			...
			->whereIn('status',[TransactionStatus::PENDING,TransactionStatus::CONFIRMED,TransactionStatus::FOR_RECEIVING])
			->get();
