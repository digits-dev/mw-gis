...
@endpush
    ...
    <div class='panel panel-default'>
        ...
        <h3 class="box-title text-center"><b>Pick Confirm Stock Transfer</b></h3>
        </div>

        <div class='panel-body'>

            <form action="{{ route('saveSTSOnlinePickConfirm') }}" method="POST" id="st_received" autocomplete="off" role="form" enctype="multipart/form-data">
            ...
            <input type="hidden" name="customer_name" id="customer_name" value="{{ $transfer_from->bea_so_store_name }}"/>
            <input type="hidden" name="from_transfer_subinventory" id="from_transfer_subinventory" value="{{ $transfer_from->sit_subinventory }}"/>
            <input type="hidden" name="transfer_to_subinventory" id="transfer_to_subinventory" value="{{ $transfer_to->sit_subinventory }}"/>
            
            <input type="hidden" name="transfer_from_store" id="transfer_from_store" value="{{ $transfer_from->bea_mo_store_name }}"/>
            <input type="hidden" name="transfer_to_store" id="transfer_to_store" value="{{ $transfer_to->bea_mo_store_name }}"/>
                                ...
                                @foreach ($items as $item)
                                    <tr>
                                        ...
                                        <td class="text-center scan scanqty{{$item['digits_code']}}">
                                            <input class="form-control text-center rcv_qty" type="text" data-id="{{ $item['digits_code'] }}" name="received_quantity[]" id="qty_{{ $item['digits_code'] }}" value="{{ $item['st_quantity'] }}" readonly/>
                                        </td>
                                        <td>
                                            @for($i = 0; $i < count($item['st_serial_numbers']); $i++)
                                                <input class="form-control" id="serial_number_{{$item['digits_code']}}{{$i}}" type="text" name="{{$item['digits_code']}}_serial_number[]" value="{{ $item['st_serial_numbers'][$i] }}" readonly/>
                                            @endfor
                                        </td>
                                    </tr>    
                                @endforeach
        ...
        <div class="panel-footer">
            ...
            <button class="btn btn-primary pull-right" type="submit" id="btnSubmit"> <i class="fa fa-save" ></i> {{ trans('message.form.pick-confirm') }}</button>
        </div>
        </form>
    </div>
